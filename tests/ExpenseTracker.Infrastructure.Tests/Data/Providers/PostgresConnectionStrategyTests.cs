...
{
    [TestFixture]
    public class PostgresConnectionStrategyTests
    {
        ...
        public void SetUp()
        {
            _connectionStrategy = new PostgresConnectionStrategy();
        }

        [Test]
        public void CreateConnection_ReturnsNpgsqlConnection_WhenConnectionStringIsValid()
        {
            // Arrange
            var connStr = "Host=localhost;Port=5432;Database=ExpenseTracker;Username=postgres;Password=yourpassword";
            ...
            {
                Assert.That(result.GetType().Name, Is.EqualTo("NpgsqlConnection"));
